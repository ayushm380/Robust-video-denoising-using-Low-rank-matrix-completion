...
%         i,j and f matching indices for a given patch with index = [i j 1]
        ...
        Omega1 = false(64,num_match*nframes);
        for k=1:nframes*5
            matchedpatches(:,k) = reshape(patches(:,mindicesi(k),mindicesj(k),mindicesf(k)),[64 1]); 
            Omega1(:,k) = reshape(Omega(:,mindicesi(k),mindicesj(k)),[64 1]);
        end
        P = matchedpatches;
        [Q] = LRMC(P,Omega1);
%         Completing matrix P using LRMC algoritm
        denoised(:,:,i,j) = Q;
    end
end
%%
